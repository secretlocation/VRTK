...
namespace VRTK
{
	...
	public class VRTK_BodyPhysics : VRTK_DestinationMarker
	{
		...
		public float headsetHeightOffset = 0.2f;
		...
		[Tooltip("The `up` distance between the headset and the object being leaned over, if object being leaned over is taller than this threshold then the current standing position won't be updated.")]
		public float leanHeightThreshold = 0.5f;
		[Tooltip("The `down` distance to check below feet for a trackable object.")]
		public float trackedObjectDepth = 0.01f;

		[Header("Step Settings")]

		[Tooltip("The maximum height to consider when checking if an object can be stepped upon to.")]
		public float stepUpHeightOffset = 0.15f;
		...
		[Tooltip("The distance between the current play area height position and the new stepped up height position to consider a valid step up. A higher number can help with juddering on slopes or small increases in collider heights.")]
		...
		[Tooltip("When the `up` distance between the floor and the headset exceeds this distance and `Enable Body Collisions` is true then the rigidbody gravity will be used instead of teleport to drop to nearest floor.")]
		public float gravityFallHeightThreshold = 1.0f;
		[Tooltip("The `up` distance between the floor and the headset that must change before a fade transition is initiated. If the new user location is at a higher distance than the threshold then the headset blink transition will activate on teleport. If the new user location is within the threshold then no blink transition will happen, which is useful for walking up slopes, meshes and terrains to prevent constant blinking.")]
		public float blinkHeightThreshold = 0.15f;
		[Tooltip("The amount the `up` position needs to change by between the current floor `up` position and the previous floor `up` position before a change in floor height is considered to have occurred. A higher value here will mean that a `Drop To Floor` will be less likely to happen if the `up` of the floor beneath the user hasn't changed as much as the given threshold.")]
		public float floorHeightTolerance = 0.001f;
		[Range(1, 10)]
		[Tooltip("The amount of rounding on the play area up position to be applied when checking if falling is occuring.")]
		...
		/// <summary>
		/// Emitted when the global orientation has changed.
		/// </summary>
		public event BodyPhysicsEventHandler OrientationChanged;
		...
		protected GameObject currentFloorObject = null;

		protected GameObject currentTrackedObject = null;
		protected GameObject lastTrackedObject = null;
		protected ITrackable<Pose> currentTrackedPose = null;

		protected float lastFrameFloorHeight;
		protected float hitFloorHeightDelta = 0f;
		...
		protected Vector3 currentStandingPosition;
		protected Vector3 lastPlayAreaPosition = Vector3.zero;
		protected Quaternion lastPlayAreaRotation = Quaternion.identity;
		protected Quaternion lastRigRotation = Quaternion.identity;
		protected List<Vector3> standingPositionHistory = new List<Vector3>();
		...
		protected bool drawDebugGizmo = true;
		...
		public virtual void ApplyBodyVelocity(Vector3 velocity, bool forcePhysicsOn = false, bool applyMomentum = false)
		{
			...
			{
				Vector3 appliedGravity = Physics.gravity.normalized * gravityPush;
				bodyRigidbody.velocity = velocity + appliedGravity;
				ApplyBodyMomentum(applyMomentum);
				StartFall(currentFloorObject);
			}
		}
		...
		public virtual void ToggleOnGround(bool state)
		{
			...
			if (onGround)
			{
				OnStartTouchingGround(SetBodyPhysicsEvent(currentFloorObject, null));
			}
			else
			{
		...
		/// <summary>
		/// The GetCurrentTrackedPose method returns the tracked pose of the body physics. 
		/// </summary>
		/// <returns>The trackable pose of the body physics</returns>
		public virtual ITrackable<Pose> GetCurrentTrackedPose()
		{
			return currentTrackedPose;
		}
		...
		{
			Vector3 point1 = bodyCollider.transform.parent.TransformPoint(bodyCollider.transform.localPosition + (bodyCollider.center)) + (VRTK_Orientation.Up * ((bodyCollider.height * 0.5f) - bodyCollider.radius));
			Vector3 point2 = bodyCollider.transform.parent.TransformPoint(bodyCollider.transform.localPosition + (bodyCollider.center)) - (VRTK_Orientation.Up * ((bodyCollider.height * 0.5f) - bodyCollider.radius));
...
#pragma warning restore 0618
		...
		protected virtual void FixedUpdate()
		{
			...
			lastPlayAreaRotation = (playArea != null ? playArea.rotation : Quaternion.identity);
		}
		...
		protected virtual void OnDrawGizmos()
		{
			if (drawDebugGizmo && headset != null)
			{
				...
				Gizmos.DrawSphere(headset.position + (VRTK_Orientation.Down * 0.3f), 0.075f);
				Gizmos.color = Color.red;
				Gizmos.DrawSphere(currentStandingPosition + (VRTK_Orientation.Down * 0.3f), 0.05f);
			}
		}

		protected virtual bool CheckValidCollision(GameObject checkObject)
		{
			return (!VRTK_PlayerObject.IsPlayerObject(checkObject) && (!onGround || (currentFloorObject != null && currentFloorObject != checkObject)));
		}
		...
		protected virtual void SetupPlayArea()
		{
			...
			if (playArea != null)
			{
				...
				lastPlayAreaRotation = playArea.rotation;
		...
		protected virtual void SetupHeadset()
		{
			...
			SetCurrentStandingPosition(true);
		}
		...
		protected virtual void OnOrientationChanged(BodyPhysicsEventArgs e)
		{
		...
		protected virtual void ManageTrackedObject()
		{
			Vector3 rayCenter = playArea.TransformPoint(footCollider.center);
			float rayLength = (footCollider.height * 0.5f) + trackedObjectDepth;
			RaycastHit hitData;

#pragma warning disable 0618
			currentTrackedObject = (VRTK_CustomRaycast.Raycast(
				customRaycast,
				new Ray(rayCenter, VRTK_Orientation.Down),
				out hitData,
				layersToIgnore,
				rayLength,
				QueryTriggerInteraction.Ignore
			) ? hitData.collider.gameObject : null);
#pragma warning disable 0618

			if (currentTrackedObject != null)
			{
				if (currentTrackedObject != lastTrackedObject)
				{
					currentTrackedPose = currentTrackedObject.GetComponent<ITrackable<Pose>>();
				}

				if (currentTrackedPose != null)
				{
					Vector3 currentPosition = currentTrackedPose.Current.position;
					Vector3 lastPosition = currentTrackedPose.Last.position;
					Vector3 deltaPosition = GetProjectedPoint(currentPosition - lastPosition);
					playArea.position += deltaPosition;
				}
			}
			else { currentTrackedPose = null; }

			lastTrackedObject = currentTrackedObject;
		}
		...
		protected virtual void CheckFalling()
		{
			if (playArea != null)
			{
				float lastHeight = GetRelativeHeight(lastPlayAreaPosition);
				float currentHeight = GetRelativeHeight(playArea.position);
				if (isFalling && fallMinTime < Time.time && VRTK_SharedMethods.RoundFloat(lastHeight, fallCheckPrecision) == VRTK_SharedMethods.RoundFloat(currentHeight, fallCheckPrecision))
				{
					StopFall();
				}
			}
		...
		protected virtual void SetCurrentStandingPosition(bool isReset = false)
		{
			if (isReset)
			{
				currentStandingPosition = (headset != null) ? headset.position : Vector3.zero;
				standingPositionHistory.Clear();
			}
			else if (playArea != null && playArea.transform.position != lastPlayAreaPosition)
			{
				currentStandingPosition += playArea.transform.position - lastPlayAreaPosition;
			}
		}

		protected virtual void SetIsMoving(Vector3 currentHeadsetPosition)
		{
			Vector3 lateralHeadsetPosition = GetProjectedPoint(currentHeadsetPosition);
			Vector3 lateralStandingPosition = GetProjectedPoint(currentStandingPosition);

			float moveDistance = Vector3.Distance(lateralHeadsetPosition, lateralStandingPosition);
		...
		protected virtual void CheckLean()
		{
			if (bodyCollider != null && headset != null)
			{
				...
				float headsetHeight = GetRelativeHeight(headset.position);
				Vector3 standingDownRayStartPosition = GetProjectedPoint(currentStandingPosition) + (VRTK_Orientation.Up * headsetHeight);
				Ray standingDownRay = new Ray(standingDownRayStartPosition, VRTK_Orientation.Down);
				RaycastHit standingDownRayCollision;

				//Determine the current valid floor that the user is standing over
#pragma warning disable 0618
				currentFloorObject = (VRTK_CustomRaycast.Raycast(customRaycast, standingDownRay, out standingDownRayCollision, layersToIgnore, Mathf.Infinity, QueryTriggerInteraction.Ignore) ? standingDownRayCollision.collider.gameObject : null);
#pragma warning restore 0618

				//Don't bother checking for lean if body collisions are disabled
				if (!enableBodyCollisions) { return; }

				//reset the headset rotation so the forward ray is always horizontal regardless of the headset rotation
				Quaternion storedRotation = headset.rotation;
				headset.rotation = Quaternion.LookRotation(GetProjectedPoint(headset.forward));
...
#pragma warning disable 0618
				if (!VRTK_CustomRaycast.Raycast(customRaycast, forwardRay, out forwardRayCollision, layersToIgnore, forwardLength, QueryTriggerInteraction.Ignore) && currentFloorObject != null)
#pragma warning restore 0618
				{
		...
		protected virtual void CalculateLean(Vector3 startPosition, float forwardLength, float originalRayDistance)
		{
			...
			float startPositionHeight = GetRelativeHeight(startPosition);
			rayDownStartPosition = GetProjectedPoint(rayDownStartPosition) + (VRTK_Orientation.Up * startPositionHeight);

			Ray downRay = new Ray(rayDownStartPosition, VRTK_Orientation.Down);
...
#pragma warning restore 0618
			{
				...
				isLeaning = (onGround && rayDownDelta > leanHeightThreshold ? true : false);
				if (isLeaning)
				{
		...
		protected virtual void UpdateStandingPosition(Vector3 currentHeadsetPosition)
		{
			...
			if (standingPositionHistory.Count > standingHistorySamples)
			{
				if (!isLeaning && currentCollidingObject == null)
				{
					...
					Vector3 lastSample = GetProjectedPoint(standingPositionHistory[standingHistorySamples]);
					for (int i = 0; i < standingHistorySamples; i++)
					{
						Vector3 currentSample = GetProjectedPoint(standingPositionHistory[i]);
						float currentDistance = Vector3.Distance(currentSample, lastSample);
		...
		protected virtual void CheckHeadsetMovement()
		{
			...
			Vector3 currentHeadsetPosition = (headset != null ? headset.position : Vector3.zero);
		...
		{
			if (playArea != null && bodyCollider != null && footCollider != null && collision.contacts.Length > 0 && collision.contacts[0].thisCollider.transform.name == footColliderContainerNameCheck)
			{
				float playAreaHeight = GetRelativeHeight(playArea.position);
				float stepHeightIncrement = 0.55f;
				float boxCastHeight = 0.01f;

				Vector3 colliderWorldCenter = playArea.TransformPoint(footCollider.center);
				Vector3 castStart = colliderWorldCenter + (VRTK_Orientation.Up * (CalculateStepUpHeightOffset() * stepHeightIncrement));
				Vector3 castExtents = new Vector3(bodyCollider.radius, boxCastHeight, bodyCollider.radius);
				Vector3 castDirection = VRTK_Orientation.Down;
				Quaternion castRotation = VRTK_SDKManager.instance.transform.rotation;
				float castDistance = GetRelativeHeight(castStart) - playAreaHeight;

				RaycastHit floorCheckHit;
				if (Physics.BoxCast(castStart, castExtents, castDirection, out floorCheckHit, castRotation, castDistance))
				{
					float floorCheckHitHeight = GetRelativeHeight(floorCheckHit.point);
					if (floorCheckHitHeight - playAreaHeight > stepDropThreshold)
					{
						if (teleporter != null && enableTeleport)
						{
							hitFloorHeightDelta = playAreaHeight - floorCheckHitHeight;
							TeleportFall(floorCheckHitHeight, floorCheckHit);
							lastFrameFloorHeight = floorCheckHitHeight;
						}
						else
						{
							playArea.position = floorCheckHit.point - (GetProjectedPoint(headset.position) - (GetProjectedPoint(playArea.position)));
						}
					}
				}
			}
		...
		{
			if (CalculateStepUpHeightOffset() > 0f)
			{
		...
		protected virtual void UpdateRig()
		{
			if (rig.rotation != lastRigRotation)
			{
				lastRigRotation = rig.rotation;
				lastFrameFloorHeight = GetRelativeHeight(playArea.position);
				SetCurrentStandingPosition(true);
				ApplyBodyVelocity(Vector3.zero);
				OnOrientationChanged(SetBodyPhysicsEvent(null, null));
			}
		}

		protected virtual void UpdateCollider()
		{
			if (bodyCollider != null && headset != null && playArea != null)
			{
				float headsetHeight = GetRelativeHeight(headset.position);
				float playAreaHeight = GetRelativeHeight(playArea.position);

				float newBodyColliderHeight = (headsetHeight - playAreaHeight) - (headsetHeightOffset + CalculateStepUpHeightOffset());
				float newBodyColliderCenterHeight = Mathf.Max((newBodyColliderHeight * 0.5f) + CalculateStepUpHeightOffset() + playAreaHeightAdjustment, bodyCollider.radius + playAreaHeightAdjustment);

				bodyCollider.height = Mathf.Max(newBodyColliderHeight, bodyCollider.radius);
				bodyCollider.center = new Vector3(headset.localPosition.x, newBodyColliderCenterHeight, headset.localPosition.z);

				if (footCollider != null)
				{
					float footThickness = bodyCollider.radius * stepThicknessMultiplier;
					footCollider.radius = footThickness;
					footCollider.height = CalculateStepUpHeightOffset();
					footCollider.center = new Vector3(headset.localPosition.x, CalculateStepUpHeightOffset() * 0.5f, headset.localPosition.z);
				}
			}
		}

		protected virtual float CalculateStepUpHeightOffset()
		{
			return stepUpHeightOffset * 2f;
		}
		...
		protected virtual bool FloorHeightChanged(float currentHeight)
		{
			float heightDelta = Mathf.Abs(currentHeight - lastFrameFloorHeight);
			return (heightDelta > floorHeightTolerance);
		}

		protected virtual bool ValidDrop(bool rayHit, RaycastHit rayCollidedWith, float floorHeight)
		{
			return (rayHit && teleporter != null && teleporter.ValidLocation(rayCollidedWith.transform, rayCollidedWith.point) && !FloorIsGrabbedObject(rayCollidedWith) && FloorHeightChanged(floorHeight));
		}

		protected virtual float ControllerHeightCheck(GameObject controllerObj)
		{
			Ray ray = new Ray(controllerObj.transform.position, VRTK_Orientation.Down);
...
#pragma warning restore 0618
			...
			float controllerObjHeight = GetRelativeHeight(controllerObj.transform.position);
			return controllerObjHeight - rayCollidedWith.distance;
		}
		...
		protected virtual bool ControllersStillOverPreviousFloor()
		{
			...
			float previousHeight = GetRelativeHeight(playArea.position);
			bool rightCheck = (rightController.activeInHierarchy && Mathf.Abs(ControllerHeightCheck(rightController) - previousHeight) < controllerDropThreshold);
			bool leftCheck = (leftController.activeInHierarchy && Mathf.Abs(ControllerHeightCheck(leftController) - previousHeight) < controllerDropThreshold);
		...
		protected virtual void SnapToNearestFloor()
		{
			...
			if (!preventSnapToFloor && (enableBodyCollisions || enableTeleport) && headset != null && headsetHeight > playAreaHeight)
			{
				Ray ray = new Ray(headset.transform.position, VRTK_Orientation.Down);
				RaycastHit rayCollidedWith;
#pragma warning disable 0618
...
#pragma warning restore 0618
				float rayCollidedWithHeight = GetRelativeHeight(rayCollidedWith.point);
				hitFloorHeightDelta = playAreaHeight - rayCollidedWithHeight;

				if (initialFloorDrop && (ValidDrop(rayHit, rayCollidedWith, rayCollidedWithHeight) || retogglePhysicsOnCanFall))
				{
					storedCurrentPhysics = ArePhysicsEnabled();
					resetPhysicsAfterTeleport = false;
					TogglePhysics(false);

					HandleFall(rayCollidedWithHeight, rayCollidedWith);
				}
				initialFloorDrop = true;
				lastFrameFloorHeight = rayCollidedWithHeight;
			}
		}

		protected virtual bool PreventFall(float hitFloorHeight)
		{
			float playAreaHeight = GetRelativeHeight(playArea.position);
			return (hitFloorHeight < playAreaHeight && ControllersStillOverPreviousFloor());
		}

		protected virtual void HandleFall(float hitFloorHeight, RaycastHit rayCollidedWith)
		{
			if (PreventFall(hitFloorHeight))
			{
			...
			else
			{
				...
				if (enableBodyCollisions && (teleporter == null || !enableTeleport || hitFloorHeightDelta > gravityFallHeightThreshold))
				{
					GravityFall(rayCollidedWith);
				}
				else if (teleporter != null && enableTeleport)
				{
					TeleportFall(hitFloorHeight, rayCollidedWith);
				}
			}
		}
		...
		protected virtual void TeleportFall(float floorHeight, RaycastHit rayCollidedWith)
		{
			StartFall(rayCollidedWith.collider.gameObject);
			GameObject currentFloor = rayCollidedWith.transform.gameObject;
			Vector3 newPosition = GetProjectedPoint(playArea.position) + (VRTK_Orientation.Up * floorHeight);
			float originalblinkTransitionSpeed = teleporter.blinkTransitionSpeed;
			teleporter.blinkTransitionSpeed = (Mathf.Abs(hitFloorHeightDelta) > blinkHeightThreshold ? originalblinkTransitionSpeed : 0f);
		...
		protected virtual Vector3 GetProjectedPoint(Vector3 point)
		{
			return (playArea != null) ? Vector3.ProjectOnPlane(point, VRTK_Orientation.Up) : point;
		}

		protected virtual float GetRelativeHeight(Vector3 point)
		{
			return (playArea != null) ? Vector3.Dot(point, VRTK_Orientation.Up) : point.y;
		}
	}
}