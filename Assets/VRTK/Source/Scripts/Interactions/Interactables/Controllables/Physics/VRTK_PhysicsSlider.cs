...
namespace VRTK.Controllables.PhysicsBased
{
    ...
    public class VRTK_PhysicsSlider : VRTK_BasePhysicsControllable
    {
        ...
        protected virtual void EmitEvents()
        {
            ...
            if (currentPosition >= (1f - minMaxThreshold) && !AtMaxLimit())
            {
                atMaxLimit = true;
                OnMaxLimitReached(payload);
            }
            else if (currentPosition <= (0f + minMaxThreshold) && !AtMinLimit())
            {
                atMinLimit = true;
                OnMinLimitReached(payload);
            }
            else if (currentPosition > minMaxThreshold && currentPosition < (1f - minMaxThreshold))
            {
                if (AtMinLimit())
                {
